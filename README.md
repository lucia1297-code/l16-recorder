...
```bash
npm test          # 69개 테스트 (통계/CSV/검증/스토리지/인증/OTP/명부/학생코드생성/자가등록/직접추가)
```
...
**학생코드 자동 생성**: "학생코드" 열을 비워두면 자동 생성됩니다 (`S1001`, `S1002`, … 순서, 이미 등록된 코드 다음 번호부터 이어짐). 직접 정한 코드를 쓰고 싶으면 그대로 입력하면 됩니다.
